module IntCode (State(..),run,runWhile,runUntilInput,step,push,pushAll,pop,clear,code,initialState) where
...
push i s = s { input = input s ++ [i] }

pushAll :: [Integer] -> State -> State
pushAll i s = s { input = input s ++ i }

-- TODO: make this work with (Maybe Integer, State) as return value
pop :: State -> (Integer, State)
pop s = case output s of
    (x:xs)  -> (x,s { output = xs })
    _       -> (0, s)
...
code :: State -> [Integer]
code State { mem } = fmap snd . M.toList $ mem

data Instruction = Add | Mult | Input | Output | JmpNZ | JmpZ | Lt | Eq | IncRelp | Halt | Other Integer deriving (Show, Read, Eq)
