module IntCode (State(..),run,runWhile,runUntilOutput,step,push,pop,popMaybe,initialState) where
...
popMaybe :: State -> (Maybe Integer, State)
popMaybe s = case output s of
    (x:xs)  -> (Just x,s { output = xs })
    _       -> (Nothing, s)
...
runUntilOutput :: State -> (Maybe Integer, State)
runUntilOutput s =
  let s' = step s
  in if halted s'
    then (Nothing, s')
